...
vec3 CalculateViewSpaceNormal(vec3 normal, mat4 cameraView) {
    mat3 normalMatrix = mat3(transpose(inverse(cameraView)));
...
void main() {
    ...
    vec3 viewSpaceNormal = CalculateViewSpaceNormal(normal, cameraData.view);
    vec3 lightDirection = CalculateLightDirection(lightingData.position, inFragPos);
    float diffuse = CalculateDiffuse(viewSpaceNormal, lightDirection);
    vec3 viewDirection = normalize(-inFragPos);
    vec3 reflectionDirection = CalculateReflectionDirection(lightDirection, viewSpaceNormal);
